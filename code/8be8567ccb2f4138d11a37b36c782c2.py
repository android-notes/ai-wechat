...
def run(args):
    ...
    try:
        ...
        set_flow_exception(e)
    ___l(9, "如果符合 <v>消息数据.get('nick_name',None)</v> <s>等于</s> <v>None</v>  则执行缩进的指令", {'debug':False})
    if if_condition_true('符合全部条件',[(消息数据.get('nick_name',None),'等于',None)]):
        pass
        ___l(10, "结束当前流程并返回参数<v>消息项数据</v>:<s>None</s>", {'debug':False})
        return {"消息项数据":(None),}
    ___l(11, "拦截缩进的指令中的错误", {'debug':False})
    try:
        pass
        ___l(12, "获取<s>消息Item项</s>的<s>子孙元素</s>并将对象保存到<v>元素</v>", {'debug':False})
        ...
        ___l(13, "执行Python代码 <s>消息数据[\"type\"] = 'text'\r\n消息数据[\"msg\"] = 元素.name</s>", {'debug':False})
        # Insert
        消息数据["type"] = 'text'
        消息数据["msg"] = 元素.name
    ...
    try:
        ...
        ___l(17, "执行Python代码 <s>消息数据[\"type\"] = 'text'\r\n消息数据[\"msg\"] = 元素.name</s>", {'debug':False})
        # Insert
        消息数据["type"] = 'text'
        消息数据["msg"] = 元素.name
        ___l(18, "获取<s>消息Item项</s>的<s>子孙元素</s>并将对象保存到<v>元素</v>", {'debug':False})
        ...
        ___l(19, "执行Python代码 <s>消息数据[\"ref\"] = 元素.name</s>", {'debug':False})
        # Insert
        消息数据["ref"] = 元素.name
    ...
    try:
        ...
        ___l(27, "执行Python代码 <s>消息数据[\"type\"] = '表情'</s>", {'debug':False})
        # Insert
        消息数据["type"] = '表情'
        ...
        set_flow_exception(e)
    ___l(37, "执行Python代码 <s>消息数据[\"type\"] = 消息Item项.name\r\n消息数据[\"msg\"] = None</s>", {'debug':False})
    # Insert
    消息数据["type"] = 消息Item项.name
    消息数据["msg"] = None
    ___l(38, "结束当前流程并返回参数<v>消息项数据</v>:<s>消息数据</s>", {'debug':False})
    return {"消息项数据":(消息数据),}
